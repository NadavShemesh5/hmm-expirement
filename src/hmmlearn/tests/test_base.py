...
class TestBaseAgainstWikipedia:
    def setup_method(self, method):
        ...
        self.frameprob = np.asarray(
            [[0.9, 0.2], [0.9, 0.2], [0.1, 0.8], [0.9, 0.2], [0.9, 0.2]]
        )
    ...
    def test_do_backward_scaling_pass(self):
        ...
        bwdlattice = _hmmc.backward_scaling(
            self.hmm.startprob_, self.hmm.transmat_, self.frameprob, scaling_factors
        )
        refbwdlattice = np.array(
            [
                [0.0661, 0.0455],
                [0.0906, 0.1503],
                [0.4593, 0.2437],
...
class TestBaseConsistentWithGMM:
    def setup_method(self, method):
        ...
        self.log_frameprob = np.log(np.random.random((n_samples, n_components)))
    ...
    def test_score_samples(self):
        ...
        gmmposteriors = np.exp(self.log_frameprob - np.tile(norm, (1, n_components)))
...
def test_stationary_distribution():
    ...
    assert h.get_stationary_distribution().T @ h.transmat_ == pytest.approx(stationary)
